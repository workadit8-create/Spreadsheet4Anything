...
export async function voidUtangPayment(
  ...
  const purchaseOrderId = String(meta.purchaseOrderId || payment.doc_id || "");
  ...
  const { data: entries, error: entriesErr } = await supabase
    ...
    .eq("source_doc_type", "UTANG_PAYMENT")
  ...
  for (const raw of (entries || []) as JournalEntryRow[]) {
    ...
    const reversalLines = buildReversalJournalLines(drafts, voidDate, poNo);
    ...
    const result = await postJournalEntry(
      supabase,
      {
        ...
        docNo: poNo,
        entryDate: voidDate,
        sourceDocType: "UTANG_PAYMENT_VOID",
  ...
  for (const rollback of lineRollbacks) {
    ...
    if (lineErr) {
      throw new Error("Gagal restore baris PO: " + lineErr.message);
    }
  }
  ...
  if (purchaseOrderId) {
    ...
    const mergedMeta = recomputePurchaseOrderPaymentMeta(
      baseMeta,
      (orderLines || []) as PurchaseLineRow[]
    );
...
export async function deleteConfirmedUtangPayment(
  supabase: SupabaseClient,
  paymentId: string
): Promise<void> {
  const { data: payment, error: payErr } = await supabase
  ...
  if (purchaseOrderId && orderMetaBefore) {
    const { data: orderLines } = await supabase
      .from("purchase_lines")
      .select("*")
      .eq("purchase_order_id", purchaseOrderId);
